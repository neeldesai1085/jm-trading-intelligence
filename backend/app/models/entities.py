...
class ImportBatch(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now)
    __table_args__ = (UniqueConstraint('user_id', 'portfolio_id', 'file_hash', name='uq_import_user_hash'), Index('ix_import_user_portfolio', 'user_id', 'portfolio_id'))
...
class ContractNote(Base):
    ...
    __table_args__ = (UniqueConstraint('user_id','portfolio_id','contract_note',name='uq_contract_user_note'), Index('ix_contract_user_portfolio_date','user_id','portfolio_id','trade_date'))
...
class SecurityLedger(Base):
    ...
    security: Mapped[str] = mapped_column(String(255), index=True)
    buy_qty: Mapped[int] = mapped_column(Integer, default=0)
    ...
    __table_args__ = (UniqueConstraint('user_id','portfolio_id','contract_note','isin','buy_qty','sell_qty','total_buy_value_after_brokerage','total_sell_value_after_brokerage',name='uq_sec'), Index('ix_sec_user_security','user_id','security'), Index('ix_sec_user_portfolio_date','user_id','portfolio_id','trade_date'))
...
class Execution(Base):
    ...
    __table_args__ = (UniqueConstraint('user_id','portfolio_id','contract_note','trade_no',name='uq_exec'), Index('ix_exec_user_date','user_id','trade_date'), Index('ix_exec_user_portfolio_date','user_id','portfolio_id','trade_date'))
...
class TradeAnnotation(Base):
    ...
    __table_args__ = (UniqueConstraint('user_id','portfolio_id','security','buy_date','sell_date',name='uq_trade_annotation'),)
