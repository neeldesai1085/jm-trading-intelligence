...
from app.models.entities import ImportBatch, ContractNote, Execution, SecurityLedger, MarketQuote, InstrumentMapping, TradeAnnotation
...
async def upload_contract_notes(files:list[UploadFile]=File(...),db:Session=Depends(get_db)):
    ...
    for f in files:
        ...
        dest=upload_dir/filename;dest.write_bytes(content)
        try:b=import_pdf(db,dest)
...
def advanced_analytics(db:Session=Depends(get_db)):return advanced(db)
...
async def refresh_quotes(db:Session=Depends(get_db)):
    h=overview(db)['holdings'];p=provider();m={x.isin:x for x in db.scalars(select(InstrumentMapping).where(InstrumentMapping.provider==p.name)).all()};instruments=[{'isin':x['isin'],'symbol':m[x['isin']].instrument_key if x['isin'] in m else None} for x in h]
    if settings.benchmark_isin and settings.benchmark_instrument_key:instruments.append({'isin':settings.benchmark_isin,'symbol':settings.benchmark_instrument_key})
    qs=await p.quotes(instruments)
    for q in qs:db.add(MarketQuote(**q))
    db.commit();return qs
@router.get('/trade-annotations')
def trade_annotations(db:Session=Depends(get_db)):
    rows=db.scalars(select(TradeAnnotation).order_by(TradeAnnotation.sell_date)).all();return [{'security':r.security,'buy_date':r.buy_date,'sell_date':r.sell_date,'strategy':r.strategy,'setup':r.setup,'regime':r.regime,'note':r.note} for r in rows]
@router.post('/trade-annotations')
def save_trade_annotation(payload:dict,db:Session=Depends(get_db)):
    ...
    row=db.scalar(select(TradeAnnotation).where(TradeAnnotation.security==security,TradeAnnotation.buy_date==buy_date,TradeAnnotation.sell_date==sell_date));values={'strategy':str(payload.get('strategy','Unclassified')).strip() or 'Unclassified','setup':str(payload.get('setup','')).strip(),'regime':str(payload.get('regime','')).strip(),'note':str(payload.get('note','')).strip() or None}
    if row:
        for k,v in values.items():setattr(row,k,v)
    else:db.add(TradeAnnotation(security=security,buy_date=buy_date,sell_date=sell_date,**values))
    db.commit();return {'status':'saved'}
